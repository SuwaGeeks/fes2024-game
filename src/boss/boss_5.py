...
class Boss5(BossBase):
    ...
    def __init__(self, w = 48, h = 48):
        ...
        self.surfaces = [pg.image.load(f"assets/boss/5_{i + 1}.png") for i in range(2)]
    ...
    def _update_1(
        ...
        if self.shot_cycle % 60 == 0:
            speed = 10
            for dr in range(0, 360, 180):
                dx = math.cos(math.radians(dr)) * self.w
                dy = math.sin(math.radians(dr)) * self.h
                x, y = self.rect.center[0]+dx, self.rect.bottom+dy,
                v_x, v_y = (self.player_x-x), (self.player_y-y)
                norm = math.sqrt(v_x**2+v_y**2)
                enemy_bullets.append(Bullet.BulletE1(x, y, speed*v_x/norm, speed*v_y/norm))
